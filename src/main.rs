...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    }
    {
        ...
        for value in *bot_infos{
            ...
            new_bot.class = match value.1["class"].as_str(){None => "".to_string(), Some(val) => val.to_string()};
            new_bot.weapons = match value.1["weapons"].as_array(){
                None => vec![],
                Some(val) => val.iter().map(|x| x.as_str().unwrap().to_owned()).collect(),
            };
            new_bot.difficulty = match value.1["difficulty"].as_i64(){None => 1, Some(val) => val};
            new_bot.weapon_restriction = match value.1["difficulty"].as_i64(){None => 1, Some(val) => val};
            new_bot.bot_attributes = match value.1["bot_attributes"].as_array(){
                None => vec![],
                Some(val) => val.iter().map(|x| x.as_str().unwrap().to_owned()).collect(),
            }; 
            new_bot.health = match value.1["health"].as_str(){None => "".to_string(), Some(val) => val.to_string()};
            new_bot.scale = match value.1["scale"].as_f64(){None => 1.0, Some(val) => val};
            new_bot.max_vision_range = match value.1["scale"].as_i64(){None => 1200, Some(val) => val};

            new_bot.class = match value.1["class"].as_str(){None => new_bot.class.to_owned(), Some(val) => val.to_string()};

            new_bot.auto_jump_min = match value.1["auto_jump_min"].as_i64(){None => 0, Some(val) => val};
            new_bot.auto_jump_max = match value.1["auto_jump_max"].as_i64(){None => 0, Some(val) => val};

            new_bot.weight = match value.1["weight"].as_f64(){None => 1.0, Some(val) => val};
            new_bot.rarity = match value.1["rarity"].as_i64(){None => 1, Some(val) => val};

            new_bot.is_boss = match value.1["is_boss"].as_bool(){None => false, Some(val) => val};
            new_bot.is_giant = match value.1["is_giant"].as_bool(){None => false, Some(val) => val};

            new_bot.attributes = match value.1["attributes"].as_array(){
                None => vec![],
                Some(val) => {
                    val.iter().map(|x| match x.as_array(){
                        Some(inner) => [inner[0].as_str().unwrap().to_owned(), inner[1].as_str().unwrap().to_owned()],
